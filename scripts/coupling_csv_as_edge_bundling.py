...
def parse_coupleds(csv_row):
    return LinkBetweenCoupled(
        csv_row[0], csv_row[1], csv_row[2]
    )  # 2020-07-05 AG: editing this to make it work with software dependencies, TODO rename all entity based naming to coupled stuff


def link_to(existing_entitys, new_link):
    if not new_link.entity in existing_entitys:
        return {
            "name": new_link.entity,
            "size": new_link.degree,
            "imports": [new_link.coupled],
        }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generates a JSON document suitable for coupling diagrams."
    )
    parser.add_argument(
        "--coupling",
        required=True,
        help="A CSV file containing the result of a coupling analysis",
    )
